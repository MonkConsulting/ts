...
ApplicationWindow {
    ...
    function fetch_projects(selectedAccountUserId) {
        ...
        db.transaction(function(tx) {
            ...
            for (var i = 0; i < result.rows.length; i++) {
                var child_projects = tx.executeSql('SELECT count(*) FROM project_project_app where parent_id = ?', [result.rows.item(i).id]);
    ...
    function fetch_sub_project(project_id) {
        ...
        db.transaction(function(tx) {
            ...
            for (var i = 0; i < child_projects.rows.length; i++) {
                subProjectsList.push({'id': result.rows.item(i).id, 'name': result.rows.item(i).name})
            }
        })
    ...
    function fetch_sub_tasks(task_id) {
        ...
        db.transaction(function(tx) {
            ...
            for (var i = 0; i < child_tasks.rows.length; i++) {
                sub_tasks_list.push({'id': result.rows.item(i).id, 'name': result.rows.item(i).name})
            }
        })
    ...
    function findParentIndex(childIndex) {
        for (let i = childIndex - 1; i >= 0; i--) {
            if (treeModel.get(i).type === "parent") {
                return i;
            }
            if (treeModel.get(i).type === "child") {
                continue;
    ...
    // function loadDataForRightPanel(rowId) {
    //     var db = LocalStorage.openDatabaseSync("myDatabase", "1.0", "My Database", 1000000);
        
    //     db.transaction(function (tx) {
    //         // Query the database for the selected row's data
    //         var result = tx.executeSql('SELECT * FROM account_analytic_line_app WHERE id = ?', [rowId]);
            
    //         if (result.rows.length > 0) {
    //             // Assuming the table has columns like 'name', 'project_id', 'task_id', etc.
    //             var rowData = result.rows.item(0); // Get the first (and only) row
    //             console.log(JSON.stringify(rowData, null, 2))
    //             var projectId = rowData.project_id;
    //             var taskId = rowData.task_id;
    //             var project = tx.executeSql('select name from project_project_app where id = ?', [projectId]);
    //             var projectName = project.rows.length > 0 ? project.rows.item(0).name : "Unknown Project";
    //             var task = tx.executeSql('select name from project_task_app where id = ?', [taskId]);
    //             var taskName = task.rows.length > 0 ? task.rows.item(0).name : "Unknown Task";
    //             menuTasks.taskId =rowData.project_id
    //             editprojectInput.text = projectName;  // Assuming 'project_name' is a column
    //             selectedProjectId = rowData.project_id;
    //         } 
    //     });
    // }
    ...
    // function dataclearswichto(args){
    //     if (listPage) {
    //         console.log("listPage exists");
    //         if (fastrow.projectInput) {
    //             console.log("projectInput exists");
    //             listPage.projectInput.text = ""; // Clear the input
    //         } else {
    //             console.log("projectInput is undefined");
    //         }
    //     } else {
    //         console.log("listPage is undefined");
    //     }
    //     elapsedTime = 0;
    //     storedElapsedTime = 0;
    ...
    //     isManualTime = false;
    //     listPage.fastrow.projectInput.text = "";
    //     selectedProjectId = 0
    ...
    StackView {
        ...
        Rectangle {
            ...
            Rectangle {
                ...
                color: "transparent" // Ensure this block is transparent
                ...
                Text {
                    ...
                    text: "Work"
                    color: "white"
                    font.pixelSize: isDesktop() ? 16 : 30
                    anchors.left: parent.left
                    // anchors.verticalCenter: parent.verticalCenter
                    anchors.leftMargin: 20
                    anchors.top: parent.top
                ...
                Rectangle {
                    ...
                    radius: isDesktop() ?10 : 25
                    color: "#CCCCCC" // Green when checked, gray when unchecked
                    // anchors.centerIn: parent
                    anchors.top: workId.bottom 
                    anchors.left: workId.right

                    // Handle (toggle button)
                    Rectangle {
                        id: switchHandle
                        width: isDesktop() ? 22 :40
                        height: isDesktop() ? 22: 40
                        radius: isDesktop() ? 11: 20
                        color: "#FFFFFF" // Red when checked, white when unchecked
                        anchors.verticalCenter: parent.verticalCenter
                        visible: mySwitch.checked ? false : true                                    
                        anchors.left: parent.left   
                    }
                    Rectangle {
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        anchors.right: parent.right
                        visible: mySwitch.checked ? true : false
                        
                    }

                        // MouseArea to toggle the switch
                        MouseArea {
                            anchors.fill: parent
                            onClicked: {
                ...
                Switch {
                    ...
                    onCheckedChanged: {
                        if (mySwitch.checked) {
                            // listPage
                            workpersonaSwitchState = false
                            stackView.push(listPage);
                            timesheetlistData()
                            console.log("Switch is ON")
                        } else {
                            stackView.push(listPage);
                            workpersonaSwitchState = true
                            timesheetlistData()
                ...
                Text {
                    ...
                    color: "white"
                    font.pixelSize: isDesktop() ? 16 : 30
                    anchors.left: switchBackground.right
                    anchors.top: parent.top
                    anchors.topMargin: isDesktop() ? 10 : 25
                    // anchors.verticalCenter: parent.verticalCenter
            ...
            Rectangle {
                width: parent.width / 3
                height: isDesktop() ? 60 : 100
                color: "transparent" // Keep it transparent
                anchors.right: parent.right
                anchors.verticalCenter: parent.verticalCenter
                                                        ...
                                                        MouseArea {
                                                            ...
                                                            onClicked: {
                                                                ...
                                                                for (var i = 0; i <= tasks_list.length; i++) {
                                                                    edittasksListModel.append(tasks_list[i])
                                                                }
                                    ...
                                    TextInput {
                                        ...
                                        MouseArea {
                                            ...
                                            onClicked: {
                                                ...
                                                var tasks_list = fetch_tasks_list((selectedSubProjectId == 0) ? selectedProjectId : selectedSubProjectId)
                                                for (var i = 0; i <= tasks_list.length; i++) {
                                                    tasksListModel.append(tasks_list[i])
                                                }
                                                // python.call("backend.fetch_options_tasks", [(selectedSubProjectId == 0) ? selectedProjectId : selectedSubProjectId] , function(result) {
                                                //     // tasksList = result;
                                                //     tasksListModel.clear();
                                    ...
                                    TextInput {
                                        ...
                                        MouseArea {
                                            ...
                                            onClicked: {
                                                ...
                                                var sub_tasks_list = fetch_sub_tasks(selectedTaskId);
                                                for (var i = 0; i <= sub_tasks_list.length; i++) {
                                                    subTasksListModel.append(sub_tasks_list[i])
                                                }
                                                menuSubTasks.open();
